...
export async function indexRevision(engine: Engine, r: AtomRevision, s: Session): Promise<number> {
  ...
  const old = engine.storage.metaGet<{ config: string; hash: string; policyId?: string }>(key);
  if (old?.config === engine.config && old.hash === digest(text) && old.policyId === r.policyId)
    return 0;
...
export async function updateIndex(
  ...
  for (const policy of s.trace.policies) {
    const key = `sdk:index-progress:${digest(canonical([engine.config, policy, s.trace.policies]))}`;
