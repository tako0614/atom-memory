...
公開API、検索、編集、ハーネスの動作を決定的なテストで確認し、実モデルの回答は別の評価として記録します。公開版の検証では82テストが合格しています。環境・コマンド・出力は [検証記録](https://github.com/tako0614/atom-memory/blob/main/validation/README.md)から参照できます。
...
Qwen2.5-7B-Instruct Q4_K_M と同じ合成資料を使い、4ステップの実行を比較しました。

| 経路                 | 入力 tokens 合計 | 必要な回答が正しかったステップ | 古い情報が入力に残ったステップ |
| -------------------- | ---------------- | ------------------------------ | ------------------------------ |
| 自動 read なし       | 810              | 0 / 4                          | 0                              |
| 履歴を追記           | 1,630            | 3 / 4                          | 2                              |
| 自動 read と記憶置換 | 1,120            | 4 / 4                          | 0                              |

回答の判定は、実装エージェントが保存した応答を確認したものです。単一の合成シナリオであり、言語指定や出力形式に従わない応答もありました。条件欠落、失敗した先行試行、モデル設定を含む [評価記録](https://github.com/tako0614/atom-memory/blob/main/validation/live-review.json)と [実行ログ](https://github.com/tako0614/atom-memory/blob/main/validation/live-evaluation.json)を公開しています。一般的なタスク成功率は別途評価が必要です。

[ローカルモデルの起動手順](/runtime#ローカルモデルで動かす)に従い、同じ接続設定で `npm run evaluate:live` を実行できます。接続先を設定していないテストでは、実 LLM は実行しません。

## 測定範囲

ローカルの語彙検索は10・100・1,000・10,000件で、ID順の最後に置いた正解が先頭になることを確認しました。関係数10・100・300では続きから全件を取得し、同じ版への同時改訂2・8・32件では一件だけが確定しました。[測定値](https://github.com/tako0614/atom-memory/blob/main/validation/local-evaluation.json)は実行環境と併せて参照してください。
