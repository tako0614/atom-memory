...
| 本文一致・利用活性・関係伝播の単一評価、誤差上限、budget、near-unit拒否   | `test/evaluation.test.mjs`                                          |
| 主体・policy・revisionごとの利用、重複ack、半減期、reset、時計後退、purge | `test/activation.test.mjs` / `test/activation-integration.test.mjs` |
...
0.5以前の `validation/*.json` は当時の環境・契約での履歴記録です。旧HarnessやSchur研究の結果をv0.6の検証済み結果へ読み替えません。質問を見せずにWriterが構造を作れるか、構造が同じ費用で検索・回答を改善するかは、それぞれ別の品質評価です。v0.6の利用ackは、成功したモデル応答をアプリが自動記録する境界までをライブラリで検証します。
