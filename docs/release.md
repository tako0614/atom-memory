...
**atom-memory 0.4.0** を npm で公開しています。Node.js 22.13 以降の ESM プロジェクトで利用でき、TypeScript の型宣言を同梱しています。
...
0.4.0では、関係とベクトルを組み合わせる共通ランキング、方向・役割別の重み、順位の内訳、ベクトル設定の独立と0.3索引の移行を追加しました。旧低水準readと旧ハーネスは廃止しました。[ランキング](/ranking)と[移行](/migration)を参照してください。
...
| ソースと配布 tarball         | [GitHub v0.4.0](https://github.com/tako0614/atom-memory/releases/tag/v0.4.0)       |
