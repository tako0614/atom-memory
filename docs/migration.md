...
基本の5操作、既存AtomのIDと版はそのまま使えます。

- `MemoryHarness` のモデル応答に `batch` を追加しました。複数Atomの作成・改訂・関係付けを一度に返せます。独自の応答バリデーターでは新しい操作も受け付けてください。過去資料の整理には、ホストが `run` に `basis: 'historical'` を渡せます。
- `host.indexAtoms(refs, binding)` と `host.updateIndex(binding)` を追加しました。既存の `prepareIndex` も使用できます。検索表現の設定が更新されるため、旧版のベクトルを再利用せず、新しい設定で索引を準備します。
- エンコーダーの `embed` に第三引数 `purpose`（`'query'` / `'document'`）を渡します。二引数の実装も引き続き動きます。
- SQLiteに依存・ベクトル候補の索引を追加しました。既存データの初回起動時には依存索引の構築時間を確保してください。`HybridCandidateProvider` は明示的に選ぶ近似検索で、既定の候補取得方式は変わりません。

[長期Writer](/history)と[保存・検索の設定](/adapters)で呼び出し例を確認できます。
