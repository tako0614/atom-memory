...
`InputToken` / `HostInput` はホストの入力確定、`ReceiptManifest` / `PresentationUnit` は版付き監査記録、`AtomLink` は安全な利用不可形を含むunion、`SelectionDiagnostics` は代理目的と作業打切りを表します。意味の正本は[規範仕様](/specification)。通常APIは五つのままです。
