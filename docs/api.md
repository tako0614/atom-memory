...
`items` は入力との類似度と関係の伝播で並べた候補です。説明と関係も候補になり、必要なら `inspect` で根拠を調べられます。`score` は順位づけの値で、事実の信頼度を表す確率ではありません。`scoreBreakdown` の `direct` と `structural` で寄与を確認できます。[計算と設定](/ranking)を参照してください。
...
`items: []` は結果がないページです。索引の反映待ちや有限範囲での近似かどうかは `diagnostics` と `cursor` を併せて確認します。
