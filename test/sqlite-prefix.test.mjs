...
test('transient cleanup does not load durable write traces', () => {
  ...
  try {
    ...
    for (let i = 0; i < 200; i++) {
      storage.metaSet(`sdk:trace:durable-${i}`, {
        id: `durable-${i}`,
        createdAt: 0,
        large: 'x'.repeat(10000),
      });
      storage.metaSet(`receipt:durable-${i}`, {
        receipt: { receiptId: `durable-${i}` },
        reads: [],
      });
    }
    ...
    storage.metaEntries = (prefix) => {
      assert.notEqual(
        prefix,
        'sdk:trace:',
        'retention must filter durable keys before loading bodies',
      );
