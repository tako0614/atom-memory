...
for (const adapter of ['memory', 'sqlite']) {
  ...
  const observed = async (f, ref, extra = {}) => {
    const inspection = await f.writer.inspect(ref, { budget, depth: 0 });
  ...
  test(`${adapter} V08-22 acquisition and presentation are distinct`, async (t) => {
    ...
    for (let i = 0; i < 12; i++) await f.memory.write(`needle ${i}`);
  ...
  test(`${adapter} V08-24 independent generations share a commit only`, async (t) => {
    const f = setup(t);
    const A = await f.memory.write('alpha input');
    const B = await f.memory.write('beta input');
    const aInput = await observed(f, A.ref);
    const bInput = await observed(f, B.ref);
    const result = await f.writer.edit(
      async (d) => [
        await d.write('alpha output', { input: aInput }),
        await d.write('beta output', { input: bInput }),
      ],
      { budget },
    );
    await f.memory.edit((d) => d.revise(A.ref, 'alpha changed'), { budget });
    const page = await f.writer.search('output', { budget, depth: 0 });
    assert.ok(!page.items.some((v) => v.ref === result.value[0].ref));
    assert.ok(page.items.some((v) => v.ref === result.value[1].ref));
  });
  test(`${adapter} V08-28 forged token is refused`, async (t) => {
    const f = setup(t);
    await assert.rejects(f.writer.write('fabrication', { input: 'input:fake' }));
    for (const invalid of ['', null, false, 0, {}])
      await assert.rejects(f.writer.write('invalid token', { input: invalid }), {
        code: 'INVALID_INPUT',
      });
    const A = await f.memory.write('source');
    const token = await observed(f, A.ref);
    await assert.rejects(f.writer.write('object token', { input: { id: token } }), {
      code: 'INVALID_INPUT',
    });
    assert.throws(
      () =>
    ...
    try {
      await assert.rejects(f.writer.write('expired', { input: token }), { code: 'CURSOR_EXPIRED' });
    } finally {
      Date.now = real;
    }
    await assert.rejects(
      f.writer.write({ text: 'fake source', provenance: { kind: 'source' } }, { input: token }),
      { code: 'INVALID_INPUT' },
    );
    f.host.purge(f.storage.metaGet(`sdk:ref:${A.ref}`).target.atomId);
    await assert.rejects(f.writer.write('purged input', { input: token }));
  });
  test(`${adapter} V08-31 independent source survives ordinary relation purge`, async (t) => {
    const f = setup(t);
    const B = await f.memory.write('target');
    const P = await f.memory.write({ text: 'independent payload', links: { related: B.ref } });
    ...
    const retired = await f.memory.edit((d) => d.retire(P.ref), { budget });
    const retiredView = await f.memory.inspect(retired.value.ref, { budget, depth: 0 });
  ...
  test(`${adapter} V08-33 unavailable required blocks read but permits inspect`, async (t) => {
    const f = setup(t);
    const B = await f.memory.write('condition');
    const P = await f.memory.write({
      text: 'conditioned payload',
      links: { when: { ref: B.ref, required: true } },
    });
    f.host.purge(f.storage.metaGet(`sdk:ref:${B.ref}`).target.atomId);
    const inspection = await f.memory.inspect(P.ref, { depth: 0, budget });
  ...
  test(`${adapter} V08-01 five operations use content-only input`, async (t) => {
    const f = setup(t);
    const first = await f.memory.write('content needle');
    ...
    const edited = await f.memory.edit((d) => d.revise(first.ref, 'new needle'), { budget });
  ...
  test(`${adapter} V08-02 shared n-ary roles and cycles use ordinary Atoms`, async (t) => {
    ...
    const R = await f.memory.write({
      text: 'relation',
      links: { member: [A.ref, B.ref], second: A.ref },
    });
    const P = await f.memory.write({ text: 'parent', links: { member: A.ref } });
    await f.memory.edit((d) => d.revise(A.ref, { text: 'alpha', links: { back: R.ref } }), {
      budget,
    });
    const page = await f.memory.search('relation', { depth: 5, budget });
    assert.ok(page.items.some((v) => v.ref === P.ref));
    assert.equal((await f.memory.inspect(R.ref, { depth: 0, budget })).atom.links.length, 3);
  });
  test(`${adapter} V08-03 reading leaves semantic revisions and use state intact`, async (t) => {
    const f = setup(t);
    const a = await f.memory.write('needle');
  ...
  test(`${adapter} V08-04 observed inspection and logical links retain separate versions`, async (t) => {
    const f = setup(t);
    const a = await f.memory.write('old');
    const p = await f.memory.write({
      text: 'parent',
      links: { logical: a.ref, observed: { ref: a.ref, at: 'observed' } },
    });
    const next = await f.memory.edit((d) => d.revise(a.ref, 'new'), { budget });
    assert.equal((await f.memory.inspect(a.ref, { budget })).atom.text, 'old');
    const view = (await f.memory.inspect(p.ref, { depth: 0, budget })).atom;
    assert.equal(view.links[0].ref, next.value.ref);
    assert.equal(view.links[1].ref, a.ref);
  });
  test(`${adapter} V08-05 edit rollback and CAS never replay callbacks`, async (t) => {
    const f = setup(t);
    const a = await f.memory.write('old');
    let calls = 0;
    await assert.rejects(
    ...
    await assert.rejects(
      f.memory.edit(
        async (d) => {
          calls++;
          await d.revise(a.ref, 'loser');
          await f.memory.edit((x) => x.revise(a.ref, 'winner'), { budget });
        },
        { budget },
      ),
  ...
  test(`${adapter} V08-07 use cannot create relevance without matching body or edge`, async (t) => {
    const f = setup(t, { activation: { propagation: 0 } });
    const no = await f.memory.write('unrelated');
    await f.memory.write('needle');
    for (let i = 0; i < 10; i++)
  ...
  test(`${adapter} V08-09 cold and warm solve the same acquired graph`, async (t) => {
    const f = setup(t);
    const a = await f.memory.write('needle');
    await f.memory.write({ text: 'parent', links: { member: a.ref } });
  ...
  test(`${adapter} V08-11 acquisition and numerical truncation are separate diagnostics`, async (t) => {
    const f = setup(t);
    const a = await f.memory.write('needle');
    await f.memory.write({ text: 'relation', links: { member: a.ref } });
  ...
  test(`${adapter} V08-12 long top-ranked body competes with short closure sets`, async (t) => {
    const f = setup(t, { activation: { propagation: 0 } });
    const long = await f.memory.write('needle '.repeat(300));
    f.host.recordUse([long.ref], f.binding, { eventId: 'fixed-comparison-event' });
    const condition = await f.memory.write('unmatched condition');
  ...
  test(`${adapter} V08-13 explanation itself can be the answer`, async (t) => {
    const f = setup(t, { activation: { propagation: 0 } });
    const p = await f.memory.write('needle explanation contains answer');
    await f.memory.write('other detail');
    const page = await f.memory.read({ query: 'needle' }, { budget });
    assert.equal(page.items[0].ref, p.ref);
  });
  test(`${adapter} V08-14 shared required body is rendered once`, async (t) => {
    const f = setup(t, { activation: { propagation: 0 } });
    const c = await f.memory.write('unique condition');
    const a = await f.memory.write({
      text: 'needle a',
  ...
  test(`${adapter} V08-15 deep required cycles close finitely`, async (t) => {
    const f = setup(t, { activation: { propagation: 0 } });
    let a = await f.memory.write('last condition');
    const first = a;
    for (let i = 0; i < 8; i++)
      a = await f.memory.write({
  ...
  test(`${adapter} V08-16 oversized closure is deferred without ref-only substitution`, async (t) => {
    const f = setup(t, { activation: { propagation: 0 } });
    const c = await f.memory.write('condition '.repeat(1000));
    const root = await f.memory.write({
      text: 'needle',
      links: { when: { ref: c.ref, required: true } },
    });
    const small = await f.memory.write('needle small');
  ...
  test(`${adapter} V08-17 overlapping verified quotations share evidence`, async (t) => {
    ...
    const a = await writer.write('AAAABBBB', { sources: [{ ref: source.ref, start: 0, end: 8 }] });
    const b = await writer.write('BBBBCCCC', { sources: [{ ref: source.ref, start: 4, end: 12 }] });
  ...
  test(`${adapter} V08-18 different generated meanings and different sources stay separate`, async (t) => {
    const f = setup(t);
    const source = await f.memory.write('needle original');
    const a = await f.writer.write('needle only with consent', { sources: [{ ref: source.ref }] });
    const b = await f.writer.write('needle never without review', {
      sources: [{ ref: source.ref }],
    });
    const other = await f.memory.write(source.text);
  ...
  test(`${adapter} V08-19 identical quote classes count max activation while retaining selected refs`, async (t) => {
    const f = setup(t, { activation: { propagation: 0 } });
    const source = await f.memory.write('quoted');
    const writer = f.host.connect({
      ...f.binding,
      actor: { type: 'agent', generatedOrigin: 'extraction' },
    });
    const a = await writer.write('quoted', { sources: [{ ref: source.ref }] });
    const b = await writer.write('quoted', { sources: [{ ref: source.ref }] });
    const root = await f.memory.write({
      text: 'needle',
      links: {
        when: [
  ...
  test(`${adapter} V08-20 exact UTF-8 display and explicit packing work limits`, async (t) => {
    const f = setup(t);
    await f.memory.write('日本語 needle');
  ...
  test(`${adapter} V08-21 small corpus oracle compares every feasible public rendering`, async (t) => {
    ...
    for (const text of ['needle long long', 'needle a', 'needle b', 'needle c'])
      views.push(await f.memory.write(text));
  ...
  test(`${adapter} V08-25 citations cannot split one generation input`, async (t) => {
    const f = setup(t);
    const A = await f.memory.write('A');
    const B = await f.memory.write('B');
    const token = f.host.observe(
    ...
    const result = await f.writer.edit(
      async (d) => [
        await d.write('needle a', { input: token, sources: [{ ref: A.ref }] }),
        await d.write('needle b', { input: token, sources: [{ ref: B.ref }] }),
      ],
      { budget },
    );
    assert.equal(f.host.manifest(token, f.agentBinding).generation.outputs.length, 2);
    await f.memory.edit((d) => d.revise(A.ref, 'A changed'), { budget });
    const page = await f.writer.search('needle', { budget, depth: 0 });
    assert.equal(page.items.length, 0);
    assert.equal(page.stale.length, result.changes.length);
  });
  test(`${adapter} V08-26 inherited working state cannot drop its dependency`, async (t) => {
    ...
    const out = await f.writer.write('needle inherited', { input: next });
    await f.memory.edit((d) => d.revise(A.ref, 'changed'), { budget });
  ...
  test(`${adapter} inherited staged generation outputs remain purge dependencies`, async (t) => {
    const f = setup(t);
    const source = await f.memory.write('ancestor source');
    ...
    const edit = await f.writer.edit(
      async (d) => [
        await d.write('parent output', { input }),
        await d.write('child output', { input: childInput }),
      ],
      { budget },
    );
    const parent = f.storage.metaGet(`sdk:ref:${edit.value[0].ref}`).target;
    assert.ok(
      f.host
    ...
    await assert.rejects(f.memory.inspect(edit.value[1].ref), { code: 'ACCESS_DENIED' });
  ...
  test(`${adapter} V08-27 unshown selection inputs remain erasure dependencies`, async (t) => {
    ...
    const out = await f.writer.write('generated result', { input: token });
  ...
  test(`${adapter} V08-29 historical and empty-query currentness differ`, async (t) => {
    const f = setup(t);
    const A = await f.memory.write('past input');
    const historical = await observed(f, A.ref, { basis: 'historical' });
    await f.memory.edit((d) => d.revise(A.ref, 'new head'), { budget });
    const out = await f.writer.write('historical needle', { input: historical });
    assert.ok((await f.writer.search('needle', { budget })).items.some((i) => i.ref === out.ref));
    const generated = await f.writer.write('past derived', {
    ...
    await f.memory.edit((d) => d.revise(latest.ref, 'newest'), { budget });
    const analysis = await f.writer.write('historical analysis', { input: historicalDerived });
    assert.ok(
    ...
    await f.memory.write('missing inserted');
    await assert.rejects(f.writer.write('depends on absence', { input: current }), {
      code: 'REVISION_CONFLICT',
    });
  });
  test(`${adapter} V08-30 tokenless agent edit retains whole-edit legacy inputs`, async (t) => {
    const f = setup(t);
    const A = await f.memory.write('A');
    const B = await f.memory.write('B');
    const out = await f.writer.edit(
      async (d) => {
        await d.inspect(A.ref, { version: 'latest', depth: 0 });
        await d.inspect(B.ref, { version: 'latest', depth: 0 });
        return [
          await d.write('needle a', { sources: [{ ref: A.ref }] }),
          await d.write('needle b', { sources: [{ ref: B.ref }] }),
        ];
      },
      { budget },
    );
    ...
    const page = await f.writer.search('needle', { budget, depth: 0 });
    assert.equal(page.items.length, 0);
    assert.equal(page.stale.length, 2);
  });
  test(`${adapter} mixed edit keeps fallback inputs without merging verified generations`, async (t) => {
    const f = setup(t);
    const A = await f.memory.write('A');
    const B = await f.memory.write('B');
    const a = await observed(f, A.ref);
    const b = await observed(f, B.ref);
    const output = await f.writer.edit(
      async (d) => [
        await d.write('needle a', { input: a }),
        await d.write('needle b', { input: b }),
        await d.write('needle fallback'),
      ],
      { budget },
    );
    await f.memory.edit((d) => d.revise(A.ref, 'changed'), { budget });
    ...
    assert.equal(purged.erasedAtomIds.length, 3);
    assert.equal((await f.writer.inspect(output.value[1].ref)).atom.text, 'needle b');
  });
  test(`${adapter} V08-32 generation and inherited erasure ignore search weights`, async (t) => {
    ...
    const B = await f.memory.write('input');
    const token = await observed(f, B.ref);
    const D = await f.writer.write('derived', { input: token });
    const child = await f.writer.write('descendant', { input: await observed(f, D.ref) });
  ...
  test(`${adapter} V08-34 hidden ordinary targets expose no identity or reason`, async (t) => {
    ...
    const B = await privateClient.write('secret');
    const publicClient = f.host.connect(wide);
    const P = await publicClient.write({ text: 'independent', links: { related: B.ref } });
  ...
  test(`${adapter} V08-35 mixed legacy manifests preserve conservative deletion`, async (t) => {
    const f = setup(t);
    const B = await f.memory.write('private input');
    const legacy = await f.writer.write({ text: 'legacy copied', links: { arbitrary: B.ref } });
    const independent = await f.memory.write({
  ...
  test(`${adapter} V08-36 failed and budgeted purge blocks reads until explicit resume`, async (t) => {
    const f = setup(t);
    const B = await f.memory.write('private bytes');
    const id = f.storage.metaGet(`sdk:ref:${B.ref}`).target.atomId;
  ...
  test(`${adapter} V08-38 reset, subjects, and revisions preserve isolation`, async (t) => {
    ...
    const next = await f.memory.edit((d) => d.revise(A.ref, 'needle revised'), { budget });
  ...
  test(`${adapter} V08-39 cursor fixes evaluation while each page includes required text`, async (t) => {
    const f = setup(t, { activation: { propagation: 0 } });
    const c = await f.memory.write('condition');
    for (const text of ['needle a', 'needle b'])
      await f.memory.write({ text, links: { when: { ref: c.ref, required: true } } });
